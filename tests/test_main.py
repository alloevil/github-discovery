...
class TestUnsubscribe:
    """#10:每封 digest 带一键退订链接与 List-Unsubscribe 头。"""

    def _payload(self, recipient="a@b.com"):
        with patch.object(main, "RESEND_API_KEY", "re_test"), \
             patch.object(main, "UNSUBSCRIBE_SECRET", "test-secret"), \
             patch("main.subprocess.run") as mrun:
            mrun.return_value = MagicMock(returncode=0, stdout=json.dumps({"id": "x"}), stderr="")
            body = f'<html><a href="{main.UNSUB_PLACEHOLDER}">Unsubscribe</a></html>'
            main.send_email_via_resend([recipient], "s", body)
        return _payload_of(mrun.call_args_list[0])

    def test_list_unsubscribe_headers_present(self):
        p = self._payload()
        headers = p["headers"]
        assert headers["List-Unsubscribe"].startswith("<https://")
        assert "action=unsubscribe" in headers["List-Unsubscribe"]
        assert headers["List-Unsubscribe-Post"] == "List-Unsubscribe=One-Click"

    def test_body_contains_recipient_specific_link(self):
        p = self._payload("a@b.com")
        with patch.object(main, "UNSUBSCRIBE_SECRET", "test-secret"):
            expected = main.unsubscribe_url("a@b.com")
        # 占位符已替换为该收件人的签名链接（HTML 属性转义后)
        assert main.UNSUB_PLACEHOLDER not in p["html"]
        assert expected.replace("&", "&amp;") in p["html"]

    def test_tokens_differ_per_recipient(self):
        with patch.object(main, "UNSUBSCRIBE_SECRET", "test-secret"):
            assert main.unsubscribe_token("a@b.com") != main.unsubscribe_token("c@d.com")
            # 大小写不敏感:GAS 端 lowercase 后验证
            assert main.unsubscribe_token("A@B.com") == main.unsubscribe_token("a@b.com")

    def test_digest_email_body_has_unsubscribe_placeholder(self):
        """digest 正文模板必须带退订占位符,发送时逐收件人替换。"""
        repo = {"full_name": "u/r", "url": "https://github.com/u/r",
                "stars": 100, "age_days": 5, "daily_stars": 10.0,
                "language": "Python", "description": "x", "source": "trending"}
        captured = {}
        def fake_send(to, subject, html):
            captured["html"] = html
            return "sent"
        with patch("main.get_subscribers", return_value=["a@b.com"]), \
             patch("main.send_email_via_resend", side_effect=fake_send):
            main.send_digest_email("2026-08-21", [(repo, {"total": 90})])
        assert main.UNSUB_PLACEHOLDER in captured["html"]
